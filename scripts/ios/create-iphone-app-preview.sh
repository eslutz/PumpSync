...
encode_segment() {
  ...
  ffmpeg -hide_banner -loglevel error -y \
    ...
    -filter_complex \
      "[0:v]reverse,trim=duration=${duration},reverse,setpts=PTS-STARTPTS,tpad=stop_mode=clone:stop_duration=${duration},trim=duration=${duration},scale=886:1920:force_original_aspect_ratio=increase,crop=886:1920,fps=30,format=yuv420p[base];[base]split=2[clean][blur-source];[blur-source]crop=${width}:${height}:${x}:${video_y},boxblur=12:1[blur];[2:v]crop=${width}:${height}:${x}:${video_y},format=gray[mask];[blur][mask]alphamerge[masked-blur];[clean][masked-blur]overlay=${x}:${video_y}:format=auto[blurred-base];[1:v]format=rgba,fade=t=in:st=${fade_start}:d=0.3:alpha=1,fade=t=out:st=${fade_end}:d=0.35:alpha=1[caption];[blurred-base][caption]overlay=x=0:y='if(lt(t,${fade_start}+0.3),12*(1-(t-${fade_start})/0.3),0)':shortest=1[out]" \
...
ffmpeg -hide_banner -loglevel error -y \
  ...
  -filter_complex \
    "[0:v]reverse,trim=duration=6,reverse,setpts=PTS-STARTPTS,tpad=stop_mode=clone:stop_duration=6,trim=duration=6,scale=886:1920:force_original_aspect_ratio=increase,crop=886:1920,fps=30,format=yuv420p[base];[base]split=2[clean][managed-source];[managed-source]crop=800:180:43:460,boxblur=12:1[managed-blur];[3:v]crop=800:180:43:460,format=gray[managed-mask];[managed-blur][managed-mask]alphamerge[masked-managed];[clean][masked-managed]overlay=43:460:format=auto[managed-base];[managed-base]split=2[clean-two][server-source];[server-source]crop=800:120:43:1180,boxblur=12:1[server-blur];[4:v]crop=800:120:43:1180,format=gray[server-mask];[server-blur][server-mask]alphamerge[masked-server];[clean-two][masked-server]overlay=43:280:format=auto[blurred-base];[1:v]format=rgba,fade=t=in:st=0.3:d=0.3:alpha=1,fade=t=out:st=2.65:d=0.3:alpha=1[managed];[2:v]format=rgba,fade=t=in:st=3.1:d=0.3:alpha=1,fade=t=out:st=5.65:d=0.35:alpha=1[server];[blurred-base][managed]overlay=x=0:y='if(lt(t,0.6),12*(1-(t-0.3)/0.3),0)':shortest=1[first];[first][server]overlay=x=0:y='if(lt(t,3.4),12*(1-(t-3.1)/0.3),0)':shortest=1[out]" \
  -map "[out]" -t 6 -an \
  -c:v libx264 -profile:v high -level:v 4.0 -preset medium -crf 15 -pix_fmt yuv420p \
...
ffmpeg -hide_banner -loglevel error -y \
  ...
  -filter_complex \
    "[0:v]reverse,trim=duration=4,reverse,setpts=PTS-STARTPTS,tpad=stop_mode=clone:stop_duration=4,trim=duration=4,scale=886:1920:force_original_aspect_ratio=increase,crop=886:1920,fps=30,format=yuv420p[base];[base]split=2[clean][blur-source];[blur-source]crop=800:460:43:280,boxblur=12:1[blur];[3:v]crop=800:460:43:280,format=gray[mask];[blur][mask]alphamerge[masked-blur];[clean][masked-blur]overlay=43:280:format=auto[blurred-base];[1:v]format=rgba,fade=t=in:st=0.45:d=0.9:alpha=1[title];[2:v]format=rgba,fade=t=in:st=1.35:d=0.9:alpha=1[tagline];[blurred-base][title]overlay=shortest=1[titled];[titled][tagline]overlay=shortest=1[out]" \
  -map "[out]" -t 4 -an \
  -c:v libx264 -profile:v high -level:v 4.0 -preset medium -crf 15 -pix_fmt yuv420p \
